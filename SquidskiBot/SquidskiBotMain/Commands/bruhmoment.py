...
class bruhmoment():
    ...
    async def isBruhMoment(self, message):
        ...
        if (myVal > .5):
            ...
            await log.logIt(f"Message wasn't a bruh moment (called by {message.author})", message)
    ...
    async def classifyBruhMoment(self, message):
        ...
        await log.logIt(f"Certified something as a bruh moment (called by {message.author})", message)
