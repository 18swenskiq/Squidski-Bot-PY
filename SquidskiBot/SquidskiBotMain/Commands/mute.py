...
class mute():
    ...
    async def mute_users(self,message):
        ...
        LoggingModule.logMessage("Muted " + str(message.mentions[0]) + " for " + message.content.split(" ")[2] + " minutes.")
        ...
        if ("579453547976982566" in str(message.mentions[0].roles)):
            ...
            LoggingModule.logMessage(str(message.mentions[0]) + " is now unmuted.")
            await message.channel.send(str(message.mentions[0]) + " has been unmuted. Please be a good user.")
        else:
            LoggingModule.logMessage("It seems that " + str(message.mentions[0]) + " was already unmuted by an admin. No message will be sent by me.")
